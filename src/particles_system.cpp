...
particles_system::particles_system(const int &start_count, const glm::vec3 &spawn, const sf::Color &color,
    ...
    for (int i = 0; i < start_count; ++i) {
        particles.push_back(particle(spawn, 1.f, speed + glm::vec3(rand()/RAND_MAX * 3, rand()/RAND_MAX * 3, rand()/RAND_MAX * 3),
                                     color));
    }
    this->spawn = spawn;
    timer_id = 0;
...
void particles_system::update_coords(const int &add_count) {
    for (int i = 0; i < add_count; ++i) {
        particles.push_back(
                particle(spawn + glm::vec3(rand()/RAND_MAX * 3, rand()/RAND_MAX * 3, rand()/RAND_MAX * 3), 1.f,
                         (*particles.begin()).speed +
                         glm::vec3(rand()/RAND_MAX * 3, rand()/RAND_MAX * 3, rand()/RAND_MAX * 3),
                         base_system_color));
    }
    for (auto &particle: particles) {
        particle.position +=
                particle.speed + glm::vec3(rand()/RAND_MAX * 3, rand()/RAND_MAX * 3, rand()/RAND_MAX * 3);
