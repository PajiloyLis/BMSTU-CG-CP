...
particles_system::particles_system(const int &start_count, const glm::vec3 &spawn, const sf::Color &color,
    ...
    particles.resize(start_count, particle(spawn, 1.f,
                                           speed + glm::vec3(2 - (rand() % 5), 2 - (rand() % 5), 2 - (rand() % 5)),
...
void particles_system::update_coords(const int &add_count) {
    for (int i = 0; i < add_count; ++i) {
        particles.push_back(particle(spawn + glm::vec3(2 - (rand() % 5), 2 - (rand() % 5), 2 - (rand() % 5)), 1.f,
                                     particles[0].speed + glm::vec3(2 - rand() % 5, 2 - rand() % 5, 2 - rand() % 5),
//                                           {static_cast<sf::Uint8 >(color.r + 1 - rand() % 3),
//                                            static_cast<sf::Uint8 >(color.g + 1 - rand() % 3),
//                                            static_cast<sf::Uint8 >(color.b + 1 - rand() % 3)}
                                     sf::Color::Black));
    ...
    for (auto &particle: particles) {
        particle.position += particle.speed + glm::vec3(2 - rand() % 5, 2 - rand() % 5, 2 - rand() % 5);
