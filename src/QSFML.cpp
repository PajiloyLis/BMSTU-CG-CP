...
void QSFMLCanvas::DrawTriangle(const triangle &t, const camera &cam, const my_vec3f &figure_center) {
    ...
    if ((cam.pos - figure_center).normalize().dot(t.n) > 0) {
        array<my_vec3f, 3> points;
        array<sf::Vertex, 3> points_to_render;
        for (int i = 0; i < points.size(); ++i) {
            points[i] = viewport(cam.perspective(cam.camLookAt(t.vertices[i], figure_center)), figure_center);
...
my_vec3f QSFMLCanvas::viewport(const my_vec3f &point, const my_vec3f &center) {
    mat4 mat(this->size().width() / (2 * center.getX()), 0, this->size().width()/2.f, 0,
             0, this->size().height() / (2 * center.getY()), this->size().height()/2.f, 0,
             0, 0, 255 / 2.f, 255/2.f,
             0, 0, 0, 1);
