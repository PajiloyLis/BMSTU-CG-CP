...
//            t2(round(points[2].x), round(points[2].y), round(points[2].z));
    ...
    for (int i = 0; i < total_height; i++) {
        ...
        int segment_height = second_half ? points[2].y - points[1].y : points[1].y - points[0].y;
        ...
        glm::vec3 A = glm::vec3(points[0]) + glm::vec3(points[2] - points[0]) * alpha;
        glm::vec3 B = (second_half ? glm::vec3(points[1]) + glm::vec3(points[2] - points[1]) * beta :
                       glm::vec3(points[0]) + glm::vec3(points[1] - points[0]) * beta);
        ...
        for (int j = round(A.x); j <= round(B.x); j++) {
            ...
            if (idx >= 0 && idx < z_buffer.size() && z_buffer[idx] < P.z) {
                glm::vec3 barycentric = glm::vec3(
                        ((points[1].y - points[2].y) * (j - points[2].x) + (points[2].x - points[1].x) * (P.y - points[2].y)) /
                        ((points[1].y - points[2].y) * (points[0].x - points[2].x) + (points[2].x - points[1].x) * (points[0].y - points[2].y)),
                        ((points[2].y - points[0].y) * (j - points[2].x) + (points[0].x - points[2].x) * (P.y - points[2].y)) /
                        ((points[1].y - points[2].y) * (points[0].x - points[2].x) + (points[2].x - points[1].x) * (points[0].y - points[2].y)),
