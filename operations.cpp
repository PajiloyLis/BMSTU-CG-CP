...
void z_buffer(array<my_vec3f, 3> points, const Image &image, sf::Color color, vector<float> &z_buffer) {
    ...
    points[0].setZ(ceil(points[0].getZ())), points[0].setX(ceil(points[0].getX())), points[0].setY(ceil(points[0].getY()));
    points[1].setZ(ceil(points[1].getZ())), points[1].setX(ceil(points[1].getX())), points[1].setY(ceil(points[1].getY()));
    points[2].setZ(ceil(points[2].getZ())), points[2].setX(ceil(points[2].getX())), points[2].setY(ceil(points[2].getY()));
        ...
        for (int j = static_cast<int>(A.getY()); j <= static_cast<int>(B.getY()); j++) {
            ...
            if (P.getZ() >= 0 && P.getY() >= 0 && P.getZ() < image.() && P.getY() < image[0].size()) {
                ...
                if (z_buffer[idx] < P.getX()) {
                    z_buffer[idx] = P.getX();
                    image[static_cast<int>(round(P.getZ()))][static_cast<int>(round(P.getY()))].color = color;
                }
            }
        }
    }
