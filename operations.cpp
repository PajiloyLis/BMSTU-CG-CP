...
z_buffer(vector<triangle> &triangles, int start, int stop, vector<vector<sf::Vertex>> &image, vector<float> &z_buffer,
    ...
    for (int k = start; k < stop; ++k) {
        float intensity = light_ray.dot(triangles[k].getN());
        if (intensity > 0) {
                ...
                for (int j = static_cast<int>(A.getY()); j <= static_cast<int>(B.getY()); j++) {
                    ...
                    if (P.getZ() >= 0 && P.getY() >= 0 && P.getZ() < image.size() && P.getY() < image[0].size()) {
                        ...
                        if (z_buffer[idx] < P.getX()) {
                            z_buffer[idx] = P.getX();
                            image[static_cast<int>(round(P.getZ()))][static_cast<int>(round(
                                    P.getY()))].color = sf::Color{static_cast<sf::Uint8>(255 * intensity),
